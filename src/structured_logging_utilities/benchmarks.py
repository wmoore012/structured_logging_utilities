...
def benchmark_logging_throughput(
    ...
    if concurrent_threads == 1:
        ...
        batch_size = len(test_messages) // concurrent_threads
        batches = [
            test_messages[i : i + batch_size] for i in range(0, len(test_messages), batch_size)
        ]

        with ThreadPoolExecutor(max_workers=concurrent_threads) as executor:
            futures = [executor.submit(log_batch, batch) for batch in batches]

            for future in as_completed(futures):
                try:
                    successful_logs += future.result()
                except Exception:
                    failed_logs += len(batches[0])  # Approximate
    ...
    avg_message_size_bytes = sum(len(json.dumps(msg)) for msg in test_messages[:100]) / 100
    ...
    return BenchmarkResult(
        ...
        metadata={
            ...
            "memory_per_log_bytes": (
                (memory_mb * 1024 * 1024) / successful_logs if successful_logs > 0 else 0
            ),
        },
    )
...
def benchmark_json_serialization(serialization_count: int = 50000) -> BenchmarkResult:
    ...
    time_seconds = end_time - start_time
    memory_mb = max(0, end_memory - start_memory)
    ops_per_second = int(successful_serializations / time_seconds) if time_seconds > 0 else 0
    success_rate = (
        successful_serializations / serialization_count if serialization_count > 0 else 1.0
    )
    ...
    return BenchmarkResult(
        ...
        metadata={
            ...
            "avg_bytes_per_serialization": (
                total_bytes / successful_serializations if successful_serializations > 0 else 0
            ),
...
def benchmark_io_performance(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for message_data in test_messages:
            try:
                ...
                if current_file_writes >= file_rotation_size:
                    ...
                    logger = get_logger(f"benchmark_io_{current_file_index}", log_file=new_log_file)
                    current_file_writes = 0
                ...
                estimated_bytes = len(
                    json.dumps({"message": message_data["message"], **message_data["extra"]})
                )
        ...
        time_seconds = end_time - start_time
        memory_mb = max(0, end_memory - start_memory)
        ops_per_second = int(successful_writes / time_seconds) if time_seconds > 0 else 0
        ...
        return BenchmarkResult(
            ...
            metadata={
                ...
                "avg_file_size_mb": (
                    (total_file_size / (1024 * 1024)) / len(log_files) if log_files else 0
                ),
                "estimated_bytes_written": total_bytes_written,
                "io_efficiency": (
                    total_file_size / total_bytes_written if total_bytes_written > 0 else 1.0
                ),
            },
        )


def benchmark_performance_context_manager(operation_count: int = 5000) -> BenchmarkResult:
    """
    ...
    for i in range(operation_count):
        try:
            with log_performance(f"test_operation_{i}", {"iteration": i}, logger=logger) as ctx:
                # Simulate some work
    ...
    time_seconds = end_time - start_time
    memory_mb = max(0, end_memory - start_memory)
    ops_per_second = int(successful_operations / time_seconds) if time_seconds > 0 else 0
    success_rate = successful_operations / operation_count if operation_count > 0 else 1.0
    ...
    overhead_per_operation_ms = (
        (overhead_seconds * 1000) / successful_operations if successful_operations > 0 else 0
    )
...
def _generate_test_log_data(count: int) -> List[Dict[str, Any]]:
    ...
    for i in range(count):
        ...
        if i % 4 == 0:
            # Simple message
            messages.append(
                {"message": f"Simple log message {i}", "extra": {"id": i, "type": "simple"}}
            )
        elif i % 4 == 1:
            # Medium complexity
            messages.append(
            ...
            data_structures.append(
                {
                    ...
                    "items": [
                        {"id": j, "value": f"item_{j}", "score": j * 0.1} for j in range(i % 10 + 1)
                    ],
...
def generate_benchmark_report(results: List[BenchmarkResult]) -> str:
    ...
    for result in results:
        ...
        if result.throughput_mb_per_second > 0:
            report.append(f"   Data throughput: {result.throughput_mb_per_second:.2f} MB/s")
            report.append("")
...
def compare_benchmark_results(
    ...
    for new_result in new_results:
        ...
        if test_name in old_lookup:
            ...
            memory_change = (
                ((new_result.memory_mb - old_result.memory_mb) / old_result.memory_mb * 100)
